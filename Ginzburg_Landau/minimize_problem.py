...
def parameter_optimization(target):
    ...
    # Minimize over mu (the period)
    mu_target = target
    d = project(Constant(mu_target), V)
    J = assemble(inner(u[3]-d, u[3]-d)*dx) / assemble(inner(d,d)*dx)
    
    # Define control parameter
    controla = Control(problem.nu)
    ...
    # Save the optimization values
    np.savetxt("results_GL.csv", np.array(rf.list_opt), delimiter=',')
    
if __name__ == "__main__":
    """
    Set the target frequency.
    The default settings correspond to reproducing Fig 2 of the paper.
    """
